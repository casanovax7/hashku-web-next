...
import { TrendingUp, Users, Target, ChevronLeft, ChevronRight } from 'lucide-react';
...
const CaseStudies = () => {
  ...
  const prevCases = () => {
    setCurrentIndex((prev) => (prev - casesPerPage + cases.length) % cases.length);
  };
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-24">
          ...
          <p className="text-xl sm:text-2xl text-gray-600 max-w-3xl mx-auto leading-relaxed">
            See how we've helped brands authentically connect with gaming audiences and drive measurable business impact.
          </p>
        </div>
              ...
              <div className="p-4 sm:p-6 relative">
                <div className="mb-6">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-xl sm:text-2xl font-bold text-black group-hover:text-gray-800 transition-colors">{study.client}</h3>
                  </div>
                  <h4 className="text-base sm:text-lg font-semibold text-yellow-600 mb-3 sm:mb-4 animate-hud-flicker">{study.title}</h4>
                  <p className="text-sm sm:text-base text-gray-600 leading-relaxed">{study.description}</p>
                </div>
              </div>
            </div>
          ))}
